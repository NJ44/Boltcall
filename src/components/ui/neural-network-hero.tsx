...
const fragmentShader = `
  ...
  void main() {
    ...
    // Transform to blue tones: reduce red, keep green low, emphasize blue
    float blueIntensity = color.b * 1.2;
    float redIntensity = color.r * 0.3;
    float greenIntensity = color.g * 0.4;
    gl_FragColor = vec4(redIntensity, greenIntensity, blueIntensity, 1.0);
  }
`;
...
function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="bg-blue-950 absolute inset-0 w-full h-full" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: -1 }} aria-hidden>
      <Canvas
        camera={camera}
...
export default function NeuralNetworkHero({
  ...
  return (
    <section ref={sectionRef} className="fixed inset-0 h-screen w-screen overflow-hidden z-0">
      <ShaderBackground />
