...
export default function AuthSwitch({
  ...
  const onSignup = async (data: SignupFormData) => {
    try {
      setIsLoading(true);
      setError("");
      await signup({ name: data.name, email: data.email, password: data.password, company: "" });
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (err instanceof Error && err.message === "OAuth redirect initiated") return;
  ...
  const socialRow = (label: string) => (
    <div className="mt-6">
      <p className="text-gray-400 text-xs text-center mb-3">
        Or {label} with social platforms
      </p>
      <div className="flex justify-center gap-3">
        {[
          { icon: <GoogleIcon />, handler: handleGoogleLogin },
          { icon: <MicrosoftIcon />, handler: handleMicrosoftLogin },
          { icon: <FacebookIcon />, handler: handleFacebookLogin },
        ].map((s, i) => (
          <button
            key={i}
            type="button"
