...
const GradientLoader = ({ className, ...props }: React.SVGProps<SVGSVGElement>) => (
  <svg
    className={cn("w-[18px] h-[18px] shrink-0 animate-spin", className)}
    viewBox="0 0 18 18"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
...
const unsavedChangesVariants = cva(
  "fixed inset-x-0 z-50 mx-auto w-fit flex items-center gap-2 rounded-full p-2 pl-3 text-sm font-medium shadow-lg shadow-black/5 border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#111114] overflow-hidden",
  {
    variants: {
      position: {
        bottom: "bottom-3",
        ...
        >
            ...
            <AnimatePresence mode="wait" initial={false}>
              {error ? (
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.2 }}
                >
                  <GradientLoader className="text-gray-400" />
                </motion.div>
              ) : (
                <motion.div
                  key="default"
                  ...
                  transition={{ duration: 0.2 }}
                >
                  {icon ?? <TriangleWarning className="text-gray-400" />}
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <AnimatePresence mode="popLayout">
            {!isSaving && !success && !error && (
              ...
              >
                <div className="absolute inset-y-0 left-0 w-2 bg-gradient-to-r from-transparent to-white dark:to-[#111114] z-20 pointer-events-none" />

                <div className="flex items-center gap-2 bg-white dark:bg-[#111114] z-10">
                  {!hideReset && (
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-7 rounded-full cursor-pointer text-xs"
                      onClick={onReset}
