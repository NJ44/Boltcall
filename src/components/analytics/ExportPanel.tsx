...
import { exportToPdf, exportFullReportPdf } from '../../lib/exportUtils';
...
const ExportPanel: React.FC<ExportPanelProps> = ({ sections }) => {
  ...
  const handleFullReportExport = async () => {
    ...
    try {
      await exportFullReportPdf(
        ALL_SECTION_IDS,
        `boltcall-full-report-${new Date().toISOString().split('T')[0]}`
      );
    } finally {
  ...
  return (
    <Card className="p-6" id="export-panel">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-brand-blue text-white rounded-lg hover:bg-brand-blueDark transition-colors disabled:opacity-50"
          >
            {exportingFull ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FileText className="w-3.5 h-3.5" />}
            {exportingFull ? 'Generating...' : 'Full Report PDF'}
          </button>
          <button
        ...
        {showHistory && (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              {history.length > 0 && (
                <button onClick={clearHistory} className="inline-flex items-center gap-1 text-xs text-red-500 hover:text-red-700">
