          ...
          <tbody>
            {agents.map((agent, i) => (
              ...
              >
                ...
                <td className="py-3 pr-4 text-right font-semibold text-text-main">
                  {agent.callsHandled.toLocaleString()}
                </td>
                <td className="py-3 pr-4 text-right text-text-muted">
                  {fmtDuration(agent.avgDurationSec)}
                </td>
                <td className="py-3 pr-4 text-right">
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${getSuccessColor(agent.successRate)}`}>
                    {agent.successRate}%
                  </span>
                </td>
