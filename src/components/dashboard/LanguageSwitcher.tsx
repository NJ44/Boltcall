...
const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({
  ...
  // ---- Icon variant (for sidebar / topbar) ----
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-1.5 p-2 rounded-lg transition-colors text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-300/30 dark:hover:bg-[#1a1a1f]"
      ...
      {open && (
        <div className="absolute bottom-full mb-2 ltr:left-0 rtl:right-0 w-48 rounded-xl shadow-xl border bg-white dark:bg-[#111114] border-gray-200 dark:border-[#2a2a30] py-1 z-50 animate-in fade-in-0 zoom-in-95">
          {SUPPORTED_LANGUAGES.map((lang) => {
