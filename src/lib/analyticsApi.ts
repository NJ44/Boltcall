...
export async function fetchFunnelData(filters: AnalyticsFilters): Promise<FunnelStage[]> {
  ...
  const currentVisits = sumField(cm, 'website_visits') || sumField(cm, 'calls') * 8;
...
export async function fetchRoiMetrics(
  ...
  const estimatedRevenue = totalLeads * config.avgDealValue * 0.3;
