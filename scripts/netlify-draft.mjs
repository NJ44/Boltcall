...
export function assertDraftArguments(args, mode = 'preview') {
  if (!['preview', 'production'].includes(mode)) throw Error('Unknown release upload mode');
  ...
  for (let index = 1; index < args.length; index++) {
    ...
    if ([mode === 'preview' ? '--draft' : '--prod', '--no-build', '--dir=dist', '--functions=.netlify-fn-build', '--timeout=600', '--json'].includes(arg)) { flags.add(arg); continue; }
    if (arg.startsWith('--cwd=') && path.isAbsolute(arg.slice(6))) continue;
    throw Error('Unexpected draft CLI argument');
  }
  if (!flags.has(mode === 'preview' ? '--draft' : '--prod') || !flags.has('--no-build')) throw Error('Only an explicit prepared deploy is allowed');
}

export function draftOnlySource(source, mode = 'preview') {
  if (sha256(source) !== DEPLOY_SITE_SHA256) throw Error('Unexpected Netlify deploy module hash');
  const original = source.toString()
    .replace('deployTimeout = DEFAULT_DEPLOY_TIMEOUT, draft = false,', `deployTimeout = DEFAULT_DEPLOY_TIMEOUT, draft = ${mode === 'preview'},`)
  ...
  const guardModule = JSON.stringify(new URL('./release-upload.mjs', import.meta.url).href);
  return `import { reportNetlifyFailure } from ${diagnosticModule};\nimport { guardPreparedUpload } from ${guardModule};\n${original}\n` +
    'export const deploySite = async (...args) => { try { return await guardPreparedUpload(args, deploySiteImplementation); } catch (error) { reportNetlifyFailure(error); throw error; } };\n';
}

export async function runDraftCLI(args, cliRoot = process.env.NETLIFY_CLI_ROOT) {
  const mode = process.env.BOLTCALL_RELEASE_MODE || 'preview';
  assertDraftArguments(args, mode);
  ...
  draftOnlySource(await fs.readFile(new URL(target)), mode);
  ...
  registerHooks({ load(url, context, nextLoad) {
    ...
    return url === target ? { ...result, source: draftOnlySource(result.source, mode) } : result;
  } });
