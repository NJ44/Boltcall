...
-- KB Folders table — groups knowledge_base documents into named folders
...
-- Join table: agents <-> kb_folders (many-to-many)
CREATE TABLE agent_kb_folders (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Add folder FK to knowledge_base
ALTER TABLE knowledge_base ADD COLUMN kb_folder_id UUID REFERENCES kb_folders(id) ON DELETE SET NULL;

-- Indexes for kb_folders
CREATE INDEX idx_kb_folders_user_id ON kb_folders(user_id);
CREATE INDEX idx_kb_folders_business_profile_id ON kb_folders(business_profile_id);
CREATE INDEX idx_kb_folders_is_default ON kb_folders(is_default);
CREATE INDEX idx_agent_kb_folders_agent_id ON agent_kb_folders(agent_id);
CREATE INDEX idx_agent_kb_folders_kb_folder_id ON agent_kb_folders(kb_folder_id);
CREATE INDEX idx_knowledge_base_kb_folder_id ON knowledge_base(kb_folder_id);

-- RLS for kb_folders
ALTER TABLE kb_folders ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view their own KB folders" ON kb_folders FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own KB folders" ON kb_folders FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own KB folders" ON kb_folders FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own KB folders" ON kb_folders FOR DELETE USING (auth.uid() = user_id);

-- RLS for agent_kb_folders
ALTER TABLE agent_kb_folders ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view their agent-folder links" ON agent_kb_folders FOR SELECT
  USING (EXISTS (SELECT 1 FROM agents WHERE agents.id = agent_kb_folders.agent_id AND agents.user_id = auth.uid()));
CREATE POLICY "Users can insert their agent-folder links" ON agent_kb_folders FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM agents WHERE agents.id = agent_kb_folders.agent_id AND agents.user_id = auth.uid()));
CREATE POLICY "Users can delete their agent-folder links" ON agent_kb_folders FOR DELETE
  USING (EXISTS (SELECT 1 FROM agents WHERE agents.id = agent_kb_folders.agent_id AND agents.user_id = auth.uid()));
